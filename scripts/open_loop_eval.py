"""Open-loop evaluation of a trained policy on the training set.

Loads a trained Pi0 checkpoint, then runs inference on selected episodes of the
LeRobot training dataset. For each episode, the policy is queried every
``action_horizon`` timesteps (matching the training action cadence) and its
stitched action predictions are compared to the ground-truth actions.

Results are logged as MSE / MAE and saved as per-episode trajectory plots.

Usage:
    python scripts/open_loop_eval.py \
      --config_name pi0_zerith \
      --checkpoint_dir ./openpi_checkpoints/pi0_zerith/<exp>/<step> \
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Training config name (e.g., 'pi0_zerith')",
    )
    parser.add_argument(
        "--checkpoint_dir",
        type=str,
        required=True,
        help="Path to checkpoint step dir (e.g., openpi_checkpoints/pi0_zerith/exp/10000)",
    )
    parser.add_argument(
        "--data_path",
        type=str,
    ...
    parser.add_argument(
        ...
        nargs="+",
        default=[0],
        help="Episode (trajectory) IDs to evaluate",
    )
    parser.add_argument(
        "--max_infer_time",
        type=int,

...
class Attention(nn.Module):
    ...
    @nn.compact
    def __call__(self, xs, positions, attn_mask, kv_cache, kv_cache_write_pos=None):
        ...
        if kv_cache is not None:
            ...
            if kv_cache_write_pos is not None:
                # Pre-allocated buffer mode: write new K/V at fixed position via
                # dynamic_update_slice (no growing concatenate → compatible with fori_loop).
                # cache_k shape: (b, max_len, k, h); new k shape: (b, T, k, h)
                start = (0, kv_cache_write_pos, 0, 0)
                k = jax.lax.dynamic_update_slice(cache_k, k, start)
                v = jax.lax.dynamic_update_slice(cache_v, v, start)
            else:
...
class Block(nn.Module):
    ...
    @nn.compact
    def __call__(self, xs, kv_cache, positions, attn_mask, adarms_cond, kv_cache_write_pos=None, deterministic=True):  # noqa: FBT002
        ...
        post_attn, kv_cache = attn(pre_attn, positions, attn_mask, kv_cache, kv_cache_write_pos)
...
class Module(nn.Module):
    ...
    def setup(self):
        ...
        block_cls = nn.remat(
            Block,
            prevent_cse=False,
            static_argnums=(5,),  # adarms_cond is static (index 5 with self, 4 after subtraction)
        ...
        self.layers = nn.scan(
            ...
            in_axes=(
                ...
                nn.broadcast,
                nn.broadcast,
            ),  # 0=kv_cache, 1=positions, 2=mask, 3=adarms_cond, 4=kv_cache_write_pos, 5=deterministic
    ...
    def __call__(
        ...
        *,
        ...
        kv_cache_write_pos: at.Int[at.Array, ""] | None = None,
        ...
        if kv_cache_write_pos is not None:
            embedded, kv_cache = self.layers(
                embedded, kv_cache, positions, mask, adarms_cond, kv_cache_write_pos, deterministic
            )
        else:
            embedded, kv_cache = self.layers(
                embedded, kv_cache, positions, mask, adarms_cond, None, deterministic
            )
